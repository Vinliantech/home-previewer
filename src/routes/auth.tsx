...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "Sign in | Kay-Steph Group Investor Portal" },
      { name: "description", content: "Sign in or create your Kay-Steph investor account." },
...
function AuthPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      // Popup success: session set, navigate.
    ...
    <PageShell>
      <section className="bg-navy pb-20 pt-[136px] text-white">
        <div className="mx-auto max-w-md px-4 sm:px-6">
          <p className="text-center text-xs font-bold uppercase tracking-[0.24em] text-gold">
            Investor portal
          </p>
          ...
          <div className="mt-8 rounded-xl bg-white p-6 text-navy shadow-2xl">
            {mode !== "forgot" && (
              <>
                <button
                  type="button"
                  ...
                  disabled={busy}
                  className="flex w-full items-center justify-center gap-3 rounded-md border border-navy/20 bg-white px-4 py-2.5 text-sm font-semibold text-navy transition hover:bg-cream disabled:opacity-60"
                >
            ...
            <div className="mt-5 flex flex-wrap justify-between gap-3 text-xs text-muted-foreground">
              {mode === "signin" && (
                <>
                  <button className="hover:text-navy" onClick={() => setMode("forgot")}>Forgot password?</button>
                  <button className="hover:text-navy" onClick={() => setMode("signup")}>Create account →</button>
                </>
              )}
              {mode === "signup" && (
                <button className="ml-auto hover:text-navy" onClick={() => setMode("signin")}>Already have an account? Sign in →</button>
              )}
              {mode === "forgot" && (
                <button className="ml-auto hover:text-navy" onClick={() => setMode("signin")}>Back to sign in →</button>
              )}
            </div>
          </div>
          ...
          <p className="mt-6 text-center text-xs text-white/60">
            ...
            <Link to="/contact" className="text-gold hover:underline">Need help?</Link>
          </p>
        </div>
      </section>
    </PageShell>
