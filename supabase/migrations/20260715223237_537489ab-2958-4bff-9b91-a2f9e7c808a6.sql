...
DO $$ BEGIN CREATE TYPE public.pool_visibility AS ENUM ('private', 'open');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN CREATE TYPE public.pool_status AS ENUM (
  'pending_approval','open','threshold_met','closing','completed','cancelled','rejected'
); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN CREATE TYPE public.pool_member_status AS ENUM (
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM public.pool_members
    WHERE pool_id = _pool_id AND user_id = _user_id AND status <> 'removed');
$$;

CREATE OR REPLACE FUNCTION public.is_pool_founder(_pool_id uuid, _user_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM public.group_pools WHERE id = _pool_id AND created_by = _user_id);
$$;
...
RETURNS TABLE (pool_id uuid, committed numeric, approved numeric, members bigint, approved_members bigint)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  WHERE p.id = ANY(_pool_ids) GROUP BY p.id;
$$;
...
CREATE OR REPLACE FUNCTION public.create_group_pool(
  _name text, _property_id uuid, _property_name text, _visibility public.pool_visibility,
  _target_amount numeric, _min_contribution numeric, _member_cap integer,
  _closing_date date, _description text, _founder_commitment numeric
) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _uid uuid := auth.uid(); _pool_id uuid;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  IF _target_amount IS NULL OR _target_amount <= 0 THEN RAISE EXCEPTION 'Target amount must be greater than zero'; END IF;
  INSERT INTO public.group_pools (name, property_id, property_name, created_by, visibility,
    target_amount, min_contribution, member_cap, closing_date, description, status)
  VALUES (_name, _property_id, _property_name, _uid, COALESCE(_visibility, 'private'),
    _target_amount, COALESCE(_min_contribution, 0), _member_cap, _closing_date, _description, 'pending_approval')
...
DECLARE _uid uuid := auth.uid(); _pool public.group_pools;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  SELECT * INTO _pool FROM public.group_pools WHERE id = _pool_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Pool not found'; END IF;
  IF _pool.status NOT IN ('open','threshold_met') THEN RAISE EXCEPTION 'This pool is not open to new members'; END IF;
  IF _pool.min_contribution > 0 AND _amount < _pool.min_contribution THEN RAISE EXCEPTION 'Contribution is below the pool minimum'; END IF;
  ...
  DO UPDATE SET committed_amount = EXCLUDED.committed_amount, status = 'pending',
...
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  IF NOT public.is_pool_founder(_pool_id, _uid) THEN RAISE EXCEPTION 'Only the pool founder can invite members'; END IF;
...
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NOT public.has_role(auth.uid(), 'admin'::public.app_role) THEN RAISE EXCEPTION 'Admins only'; END IF;
  UPDATE public.group_pools
  SET status = CASE WHEN _approve THEN 'open'::public.pool_status ELSE 'rejected'::public.pool_status END,
      admin_notes = _notes, updated_at = now()
  WHERE id = _pool_id;
END; $$;
