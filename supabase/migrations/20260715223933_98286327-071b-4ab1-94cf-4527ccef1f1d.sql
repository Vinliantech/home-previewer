-- Estate operations (Phase C sales ops): estates, plots, allocations, applications,
-- reservations, payment records/requirements, company accounts, documents, support.

-- Add 'manager' role for user role management
...
-- Grants + RLS enable + admin-all policies
DO $$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY[
    ...
    EXECUTE format(
      'CREATE POLICY %I_admin_all ON public.%I FOR ALL TO authenticated USING (public.has_role(auth.uid(), ''admin''::public.app_role)) WITH CHECK (public.has_role(auth.uid(), ''admin''::public.app_role));',
      t, t
    );
  END LOOP;
END $$;

-- Owning-user read access
...
-- Broadly readable reference tables (any authenticated user)
